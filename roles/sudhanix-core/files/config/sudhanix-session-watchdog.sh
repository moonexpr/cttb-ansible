...
if [ -z "$WM_PID" ]; then
    log "xfwm4 never appeared within 30 s — exiting watchdog (non-fatal)"
    exit 0
fi
...
log "xfwm4 confirmed dead — collecting diagnostics"
...
# User-visible message before logout.
