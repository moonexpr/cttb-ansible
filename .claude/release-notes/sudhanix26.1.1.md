...
A maintenance release. No change to the deployed desktop image — `sudhanix_release` remains `26` and the Ubuntu 24.04 (noble) base is untouched, so a host built from 26.1.0 and one built from 26.1.1 are the same machine. What changed is the wiki's appearance and, mostly, the sysadmin's experience of the repository itself.
