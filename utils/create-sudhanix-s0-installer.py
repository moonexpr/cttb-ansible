...
HOSTNAME_RE = re.compile(r"^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$", re.I)
...
def build_config(args) -> Config:
    ...
    if not HOSTNAME_RE.match(hostname):
        die(f"{hostname!r} is not a valid DNS label")
...
def sha256_head(path: str, nbytes: int, *, sudo: bool = False) -> str:
    if sudo:
        out = subprocess.run(
            ["sudo", "dd", f"if={path}", "bs=1m", f"count={nbytes // (1024 * 1024)}"],
...
def write_usb(cfg: Config) -> None:
    ...
    run(["sudo", "dd", f"if={cfg.output_iso}", f"of={cfg.raw_disk}", "bs=4m"])
    run(["sync"])
